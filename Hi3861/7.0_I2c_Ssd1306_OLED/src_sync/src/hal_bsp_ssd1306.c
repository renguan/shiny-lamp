...
/**
 * @brief SSD1306 OLED显示屏显示16x16中文字符串(UTF-8)
 * @param x X轴坐标 0~112
 * @param y Y轴坐标 0~3
 * @param ch 显示的UTF-8中文字符串
 * @return none
 */
void SSD1306_ShowCN(uint8_t x, uint8_t y, uint8_t ch[])
{
    uint8_t i = 0, j = 0, k = 0;
    uint8_t y2 = y * 2;
    /* 汉字UTF-8编码索引表(与F16x16字库一一对应): 鸿 蒙 先 锋 号 */
    const unsigned char CN16x16_Index[][3] = {
        {0xE9, 0xB8, 0xBF},   /* 鸿 */
        {0xE8, 0x92, 0x99},   /* 蒙 */
        {0xE5, 0x85, 0x88},   /* 先 */
        {0xE9, 0x94, 0x8B},   /* 锋 */
    ...
    while (ch[i] != '\0')
    {
        /* 仅处理UTF-8汉字(首字节>=0xE4), 其他字节跳过 */
        if (ch[i] >= 0xE4 && ch[i + 1] != '\0' && ch[i + 2] != '\0')
        {
            for (j = 0; j < sizeof(CN16x16_Index) / sizeof(CN16x16_Index[0]); j++)
            {
                if (ch[i] == CN16x16_Index[j][0] &&
                    ch[i + 1] == CN16x16_Index[j][1] &&
                    ch[i + 2] == CN16x16_Index[j][2])
                {
                    if (x > 112) { x = 0; y2 += 2; }
                    SSD1306_SetPos(x, y2);
                    for (k = 0; k < 16; k++)
                        SSD1306_WiteData(F16x16[j][k]);
                    SSD1306_SetPos(x, y2 + 1);
                    for (k = 0; k < 16; k++)
                        SSD1306_WiteData(F16x16[j][k + 16]);
                    x += 16;
                    break;
                }
            }
